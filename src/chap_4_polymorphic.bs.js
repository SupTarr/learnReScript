...
var lowercaseLanguages = [];

languagesAndAuthors.map(function (param) {
      return lowercaseLanguages.push(param[0].toLowerCase());
    });
...
names.map(function (param) {
      return fullnames.push(param[0] + " " + param[1]);
    });
...
var dynamicLanguages = [];

languagesAndAuthors.map(function (param) {
      var language = param[0];
      var match = compiledLanguages.includes(language);
      if (match) {
        return 0;
      } else {
        return dynamicLanguages.push(language);
      }
    });
...
function myCustomMap(list, callBack) {
  if (list) {
    return {
            hd: Curry._1(callBack, list.hd),
            tl: myCustomMap(list.tl, callBack)
          };
  } else {
    return /* [] */0;
  }
...
function myCustomFilter(_list, condition) {
  while(true) {
    var list = _list;
    if (!list) {
      return /* [] */0;
    ...
    var rest = list.tl;
    var head = list.hd;
    var match = Curry._1(condition, head);
    if (match) {
      return {
              hd: head,
              tl: myCustomFilter(rest, condition)
            };
    }
    _list = rest;
    continue ;
